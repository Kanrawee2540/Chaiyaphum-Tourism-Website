...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM notifications WHERE user_id = ? AND is_read = 0 ORDER BY created_at DESC LIMIT 1");
            ...
            <div class="flex-1 p-6 flex flex-col justify-between">
                ...
                <div class="mt-6 flex items-center">
                    <div class="flex-shrink-0">
                        <span class="sr-only"><?php echo htmlspecialchars($article['username']); ?></span>
                    </div>
                    <div class="ml-3">
...
function showNotification(message) {
    // โค้ดสำหรับแสดง popup หรือแจ้งเตือน
    alert(message); // หรือใช้ library อื่นๆ เช่น SweetAlert2
}
        // Toggle mobile menu

...
// ฟังก์ชันใหม่สำหรับตรวจสอบว่ามีการลบบทความหรือไม่
function hasDeletedArticles($conn, $user_id) {
    $stmt = $conn->prepare("SELECT 1 FROM deleted_articles_notifications WHERE admin_id = ? AND is_read = 0 LIMIT 1");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    return $result->num_rows > 0;
...
if (isset($_SESSION['user_id'])) {
    ...
    $hasDeletedArticles = hasDeletedArticles($conn, $user_id);

    if (isset($_GET['comments_viewed'])) {
        markAllCommentsAsRead($conn, $user_id);
        $hasUnreadComments = false;
    }
    if (isset($_GET['articles_deleted_viewed'])) {
        markAllDeletedArticlesAsRead($conn, $user_id);
        $hasDeletedArticles = false;
    }
} else {
    $hasUnreadComments = false;
    $hasDeletedArticles = false;
        ...
        <div class="flex items-center justify-between flex-wrap">
            ...
            <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
                <div class="text-base lg:flex-grow">
                    ...
                    <a href="manage_articles.php?articles_deleted_viewed=1" class="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-blue-200 mr-6 transition duration-300 ease-in-out relative">
                จัดการบทความ
                <?php if ($hasDeletedArticles): ?>
...
document.querySelector('a[href^="manage_articles.php"]').addEventListener('click', function(e) {
    e.preventDefault();
    this.querySelector('span')?.remove(); // ลบวงกลมสีแดง
    fetch(this.href) // ส่งคำขอไปยัง manage_articles.php เพื่ออัพเดตสถานะ
        .then(() => window.location.href = 'manage_articles.php'); // ไปยังหน้าจัดการบทความ
});
